...
| Denial of service, or being used to flood the Internet Archive | 30 lookups per minute per visitor (IPv6 grouped by network), a service-wide cap of 60 archive requests a minute that cached answers skip, at most 12 outbound requests per lookup, response size caps, and timeouts. Following the [archive's rules for bots](https://archive.org/developers/bots.html), a 429 is never retried: the whole service pauses for as long as `Retry-After` asks (a minute if it doesn't say), and a long `Retry-After` on a server error is treated the same way | `test/security/rate-limit.test.ts`, `test/security/archive-politeness.test.ts`, `test/security/response-limits.test.ts` |
